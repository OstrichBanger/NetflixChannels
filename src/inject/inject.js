    	...
    	for (var key in result){
   			...
   			if (key.substring(0,5) == "chan-") {
   				...
   				if (containCheck.indexOf(checkKey) > -1) {
   					var addButton = "<span class='addChannel btnWrap mltBtn mltBtn-s60'><a id='" + key + "' class='chandel svf-button svfb-silver addlk evo-btn save2add svf-button-add'><span class='remove'>Remove from " + val + "</span></a></span>";
   					$('.actions').append(addButton);
   					$(".chandel").each(function () {
						...
						document.getElementById(key).addEventListener('click',function () {
							...
							chrome.storage.sync.get(getShows,function (showNames){
								...
								document.getElementById(key).innerHTML="<span class='inr'>Removed from " + val + "</span>";
							});
						});
					});
   				}		
   				else {
    				var addButton = "<span class='addChannel btnWrap mltBtn mltBtn-s60'><a id='" + key + "' class='chanadd svf-button svfb-silver addlk evo-btn save2add svf-button-add'><span class='inr'>Add to " + val + "</span></a></span>";
					$('.actions').append(addButton);
					$(".chanadd").each(function () {
						var key = $(this).attr('id');
						var val = key.split('-')[1];
						document.getElementById(key).addEventListener('click',function () {
							Obj = {};
